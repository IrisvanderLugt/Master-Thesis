...
head(actuals_preds)
rmse(Pred, testData$Line.203) #18.88984
...
#Save the model
saveRDS(knnMod,".knnMod_306.rds")

#Make predictions
Pred <- predict(knnMod306, testData)

#Evaluate the predictions
actuals_preds <- data.frame(cbind(actuals=testData$Line.306, predicteds=Pred))
head(actuals_preds)
rmse(Pred, testData$Line.306) #44.31107
...
head(actuals_preds)
rmse(Pred, testData$Line.949) #130.5497
...
head(actuals_preds)
rmse(Pred, testData$Line.414) #139.9865
...
head(actuals_preds)
rmse(Pred, testData$Line.490) #89.87542
...
ggplot(actuals_preds, aes(x=actuals, y=predicteds)) +
  ...
  ggtitle("Prediction accuracy kNN model for line 490") +
  xlab("Actual values (MW)") +
  ylab("Predicted values (MW)")


#Variable importance
library(vip)
p1 <- vip(knnMod, num_features = 10)
p2 <- vip(knnMod, num_features = 20)
