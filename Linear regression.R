...
head(actuals_preds)
rmse(Pred, testData$Line.203) #28.9433
...
head(actuals_preds)
rmse(Pred, testData$Line.306) #59.61953
...
head(actuals_preds)
rmse(Pred, testData$Line.949) #209.2082
...
head(actuals_preds)
rmse(Pred, testData$Line.414) #206.5713
...
head(actuals_preds)
rmse(Pred, testData$Line.490) #129.1127


#Visualization
library(ggplot2)
dev.off()
ggplot(testData, aes(Line.490, Pred)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1) +
  ggtitle("Prediction accuracy Random Forest model for line 490")


#Variable importance
library(vip)
p1 <- vip(lmMod, num_features = 10)
p2 <- vip(lmMod, num_features = 20)
