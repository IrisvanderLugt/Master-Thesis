...
#Save the model
saveRDS(MARSMod,".marsMod_203.rds")
...
head(actuals_preds)
rmse(Pred, testData$Line.203) #28.93152
...
#Save the model
saveRDS(marsMod,".marsMod_306.rds")

#Make predictions
Pred <- predict(marsMod, testData)

#Evaluate predictions
actuals_preds <- data.frame(cbind(actuals=testData$Line.306, predicteds=Pred))  # make actuals_predicteds dataframe.
head(actuals_preds)
rmse(Pred, testData$Line.306) #60.40983
...
#Save the model
saveRDS(marsMod,".marsMod_949.rds")
...
head(actuals_preds)
rmse(Pred, testData$Line.949) #218.9737
...
#Save the model
saveRDS(marsMod,".marsMod_414.rds")
...
head(actuals_preds)
rmse(Pred, testData$Line.414) #211.3784
...
#Save the model
saveRDS(marsMod,".marsMod_490.rds")

#Make predictions
Pred <- predict(marsMod, testData)

actuals_preds <- data.frame(cbind(actuals=testData$Line.490, predicteds=Pred))  # make actuals_predicteds dataframe.
head(actuals_preds)
