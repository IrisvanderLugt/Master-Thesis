...
data203$Line.314 <- NULL

#Set Time variabel to POSIX
...
# Build the SVR model
svrMod <- svm(Line.203 ~ ., data=trainingData)

#Save the model
saveRDS(svrMod,".svrMod_203.rds")
...
# Build the SVR model
svrMod <- svm(Line.306 ~ ., data=trainingData)

#Save the model
saveRDS(svrMod,".svrMod_306.rds")
...
#Build the SVR model
svrMod <- svm(Line.949 ~ ., data=trainingData)  

#Save the model
saveRDS(svrMod,".svrMod_949.rds")
...
# Build the SVR model
svrMod <- svm(Line.414 ~ ., data=trainingData) 

#Save model
saveRDS(svrMod,".svrMod_414.rds")
...
# Build the SVR model
svrMod <- svm(Line.490 ~ ., data=trainingData) 

#Save model
saveRDS(svrMod,".svrMod_490.rds")
